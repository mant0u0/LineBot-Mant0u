...
def random_okamikuji_main(event, userMessage):
    ...
    if userMessage == "抽籤":
        ...
        flex_message = FlexSendMessage(
            alt_text="有人抽籤囉！",
            contents={"type": "carousel", "contents": flex_message_contents},
                ...
                items=[
                    ...
                    QuickReplyButton(
                        action=MessageAction(
                            label="○ " + sentence_1, text="抽籤：" + sentence_1
                        )
                    ),
                    QuickReplyButton(
                        action=MessageAction(
                            label="○ " + sentence_2, text="抽籤：" + sentence_2
                        )
                    ),
                    QuickReplyButton(
                        action=MessageAction(
                            label="○ " + sentence_3, text="抽籤：" + sentence_3
                        )
                    ),
                ]
            ),
    ...
    if userMessage.find("抽籤：") == 0:
        ...
        okamikuji_fortune = get_okamikuji_fortune(
            okamikuji_result["result_name"], userMessage
        )
        ...
        pageTemplate = random_okamikuji_fortune_page_template(
            okamikuji_result, okamikuji_fortune, userMessage
        )
        flex_message_contents.append(pageTemplate)
        ...
        flex_message = FlexSendMessage(
            ...
            quick_reply=QuickReply(
                items=[
                    QuickReplyButton(
                        action=MessageAction(
                            label="● 再問一次", text="抽籤：" + userMessage
                        )
                    ),
                    QuickReplyButton(
                        action=MessageAction(label="● 再抽一次", text="抽籤")
                    ...
                    QuickReplyButton(
                        action=MessageAction(
                            label="改塔羅牌 ➜", text="塔羅牌：" + userMessage
                        )
                    ),
                    QuickReplyButton(
                        action=MessageAction(
                            label="改用擲筊 ➜", text="擲筊：" + userMessage
                        )
                    ),
                ]
            ),
...
def get_okamikuji_fortune(result, userMessage):
    ...
    system_prompt = "你是日本神社抽籤解籤助手，請使用台灣繁體中文。只回覆一段 2~3 句話，總長度最多 100 個中文字。第一句直接說明此籤對這個主題的運勢，第二、三句提供一個具體、溫和的建議。不要標題、條列、Markdown、前言、結語，不要重述籤名或使用者問題，也不要提及 AI。"
    record_prompt = [
        {
            "user": "大吉：我的戀愛運勢",
